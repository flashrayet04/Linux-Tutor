...
function getUsernameError(val) {
    ...
    if (containsBannedWord(val))       return "⚠️ That username contains offensive language. Please choose another.";
    return null;
}
...
function buildFriendRow(person, type) {
    ...
    if (type === "friend") {
        ...
        row.innerHTML = `
            ...
            <div class="friend-actions">
                <button class="friend-action-btn friend-action-btn--accept" title="Accept">✓</button>
                <button class="friend-action-btn friend-action-btn--decline" title="Decline">✕</button>
            </div>
        `;
...
// ponytail: class→plain object; ceiling = no prototype reuse if multiple instances needed
const vfs = {
...
const RANKS = [
    ...
    { name: "Directory Artisan", icon: "🛠️", minXP: 250 },
    { name: "Terminal Specialist", icon: "⚡", minXP: 500 },
    { name: "System Guardian", icon: "🛡️", minXP: 850 },
...
const ACHIEVEMENTS = [
    ...
    { id: "grep_detective", title: "Grep Detective 🕵️", desc: "Filter logs with grep.", icon: "🕵️", unlocked: false },
    ...
    { id: "time_traveler", title: "Time Traveler 🕰️", desc: "Check your command history.", icon: "🕰️", unlocked: false }
];
...
// ponytail: if-chain → lookup table; ceiling = no partial-match ranking
const CMD_EXPLANATIONS = [
    [/^pwd$/,              "📍 <strong>Plain English:</strong> Prints your current working folder location."],
    [/^ls/,               "👀 <strong>Plain English:</strong> Lists all files and subfolders in your working location."],
    [/^cd/,               "🚶 <strong>Plain English:</strong> Changes your current folder location."],
    [/^mkdir/,            "🏗️ <strong>Plain English:</strong> Creates a brand new folder on disk."],
    [/^touch/,            "📄 <strong>Plain English:</strong> Creates a new blank file."],
    [/^cp/,               "📋 <strong>Plain English:</strong> Copies a file to a new destination."],
    [/^mv/,               "📦 <strong>Plain English:</strong> Moves or renames a file or folder."],
    [/^rm/,               "🗑️ <strong>Plain English:</strong> Permanently deletes a file or folder."],
    [/^cat/,              "📜 <strong>Plain English:</strong> Reads and prints full file text to screen."],
    [/^head/,             "🔝 <strong>Plain English:</strong> Shows the top few lines of a file."],
    [/^tail/,             "🔚 <strong>Plain English:</strong> Shows the bottom (latest) lines of a file."],
    [/^grep/,             "🕵️ <strong>Plain English:</strong> Searches line-by-line for matching text keywords."],
    [/^find/,             "🔍 <strong>Plain English:</strong> Searches across directories for matching file names."],
    [/^chmod/,            "🔒 <strong>Plain English:</strong> Updates security Read/Write/Execute permissions."],
    [/>/,                 "✍️ <strong>Plain English:</strong> Redirects text output into a file."],
    [/\|/,               "🔀 <strong>Plain English:</strong> Pipes the output of one command into another!"],
    [/^whoami/,           "👤 <strong>Plain English:</strong> Confirms your logged-in user account."],
    [/^ps/,               "⚡ <strong>Plain English:</strong> Lists active processes running in memory."],
    [/^(df|free)/,        "📊 <strong>Plain English:</strong> Checks system storage or RAM memory health."],
    [/^(bash|\.\/)/,       "🚀 <strong>Plain English:</strong> Executes an automated shell script!"],
    [/^export/,           "🧬 <strong>Plain English:</strong> Creates a reusable variable in your shell session."],
    [/^env$/,             "🌎 <strong>Plain English:</strong> Lists every environment variable currently set."],
    [/^history/,          "🕰️ <strong>Plain English:</strong> Shows every command you've typed this session."],
    [/^alias/,            "🔗 <strong>Plain English:</strong> Creates a shortcut nickname for a longer command."],
    [/^man/,              "📖 <strong>Plain English:</strong> Opens a quick reference manual for a command."],
    [/^sort/,             "📶 <strong>Plain English:</strong> Arranges the lines of a file alphabetically (or reversed)."],
    [/^uniq/,             "🧹 <strong>Plain English:</strong> Collapses adjacent duplicate lines into one."],
    [/^date$/,            "📅 <strong>Plain English:</strong> Prints the current date and time."],
    [/^uname/,            "🖥️ <strong>Plain English:</strong> Reports kernel version and system architecture."],
    [/^du/,               "💾 <strong>Plain English:</strong> Reports how much disk space your files use."],
    [/^kill/,             "⚔️ <strong>Plain English:</strong> Sends a signal to stop a running process."],
];
function getPlainEnglishExplanation(cmdStr) {
    const t = cmdStr.trim();
    return (CMD_EXPLANATIONS.find(([re]) => re.test(t)) || [, "💡 <strong>Plain English:</strong> Command executed in sandbox."])[1];
}
...
const tacticalMissions = [
    { id: "m1", title: "Mission 1: Missing Log Investigator 🕵️‍♂️", icon: "🔍", desc: "Search `logs/system.log` for ERROR entries and write to `error_report.txt`.", task: "grep ERROR logs/system.log > error_report.txt", xp: 150 },
...
const DISTROS = [
    ...
    { id: "ubuntu",    logo: "🟠", name: "Ubuntu",        tag: "Huge community/support",  badge: "⭐ Recommended", url: "https://ubuntu.com/download/desktop",           videos: ["mint"] },
    { id: "popos",     logo: "🚀", name: "Pop!_OS",       tag: "Gaming & Nvidia GPUs",    badge: null,             url: "https://pop.system76.com/",                    videos: [] },
    { id: "zorin",     logo: "🪟", name: "Zorin OS",      tag: "Closest to Windows look", badge: null,             url: "https://zorin.com/os/download/",               videos: [] },
    { id: "fedora",    logo: "🎩", name: "Fedora",        tag: "Newer software/devs",     badge: null,             url: "https://fedoraproject.org/workstation/download",videos: [] },
    { id: "debian",    logo: "🌀", name: "Debian",        tag: "Stability & servers",     badge: null,             url: "https://www.debian.org/distrib/",              videos: [] },
    { id: "elementary",logo: "🍎", name: "elementary OS", tag: "Mac-like clean design",   badge: null,             url: "https://elementary.io/",                       videos: [] },
    { id: "manjaro",   logo: "🧡", name: "Manjaro",       tag: "Arch-based, up-to-date",  badge: null,             url: "https://manjaro.org/download/",                videos: [] },
    { id: "opensuse",  logo: "🦎", name: "openSUSE",      tag: "Enterprise-style stable", badge: null,             url: "https://get.opensuse.org/",                    videos: [] },
];
...
const INSTALL_STEPS = [
    {
        ...
        body: `
            ...
            <div class="step-warn">⚠️ Always back up your personal data before proceeding — partitioning is irreversible!</div>
        `
    },
    {
        num: 2,
    ...
    },
    {
        ...
        body: `
            ...
            <div class="step-warn">⚠️ "Erase disk" will delete everything on that drive. Double-check you've backed up first!</div>
        `
    },
    {
        num: 6,
...
function processCommand(cmdStr) {
    ...
    if ($("decoder-output")) $("decoder-output").innerHTML = getPlainEnglishExplanation(cmdStr);
...
function unlockBadge(badgeId) {
    ...
    if (badge && !state.unlockedBadges.has(badgeId)) {
        ...
        badge.unlocked = true;
        sfx.playLevelUp();
        printToTerminal(`🎖️ UNLOCKED BADGE: ${badge.title}!`, "success");
        renderRanksAndBadges();
    }
}
...
function renderRanksAndBadges() {
    ...
    if (badgesGrid) {
        ...
        ACHIEVEMENTS.forEach(b => {
            ...
            card.innerHTML = `<div class="badge-card-icon">${b.icon}</div><div class="badge-card-info"><h4>${b.title} ${b.unlocked ? "✓" : "🔒"}</h4><p>${b.desc}</p></div>`;
